...
{
    public class MotorOkapiBM25F : ISearchEngine
    {
        //nombre del motor
        String Name = "2012-02.1";
        ...
        public IRun<IListResult> RunModulo1(string dbConnectionString, DocumentCollection docCol, Topic topic)
        {
            ...
            Dictionary<string, double> contentLength = new Dictionary<string, double>();
            Dictionary<string, double> boldLength = new Dictionary<string, double>();
            Dictionary<string, double> italicLength = new Dictionary<string, double>();
            Dictionary<string, double> headerLength = new Dictionary<string, double>();
            Dictionary<string, double> titleLength = new Dictionary<string, double>();
            Dictionary<string, double> descriptionLength = new Dictionary<string, double>();
            Dictionary<string, double> keywordsLength = new Dictionary<string, double>();
            double docsLengthAvg = 0;
            double contentLengthAvg = 0;
            double boldLengthAvg = 0;
            double italicLengthAvg = 0;
            double headerLengthAvg = 0;
            double titleLengthAvg = 0;
            double descriptionLengthAvg = 0;
            double keywordsLengthAvg = 0;
            System.Data.SqlClient.SqlConnection conexion = new System.Data.SqlClient.SqlConnection(dbConnectionString);
            conexion.Open();
            System.Int32 idTerm;
            string docName;
            ...
            double b = 0.75;
            int countTermsInt;
            double pesosContent = 1.0;
            double pesosBold = 50.0;
            double pesosItalic = 30.0;
            double pesosHeader = 15.0;
            double pesosTitle = 20.0;
            double pesosKeywords = 150.0;
            double pesosDescription = 5.0;
            double wContent;
            double wBold;
            double wItalic;
            double wHeader;
            ...
            System.Data.SqlClient.SqlCommand termFrecuency = new System.Data.SqlClient.SqlCommand("SELECT D.Name, TF.Tf_Value, TF.Tf_b, TF.Tf_i, TF.Tf_h, TF.Tf_title, TF.Tf_keywords, TF.Tf_description FROM DocTermFrecuencies AS TF INNER JOIN Documents AS D ON TF.[Document] = D.Iddoc WHERE  (TF.Term = @idterm)", conexion);
            ...
            System.Data.SqlClient.SqlCommand docLength = new System.Data.SqlClient.SqlCommand("SELECT D.Name, SUM(TF.Tf_Value), SUM(TF.Tf_b), SUM(TF.Tf_i), SUM(TF.Tf_h), SUM(TF.Tf_title), SUM(TF.Tf_keywords), SUM(TF.Tf_description) FROM DocTermFrecuencies AS TF INNER JOIN Documents AS D ON TF.[Document] = D.Iddoc GROUP BY D.Name", conexion);
            ...
            {
                auxLength = docLengthQuery.GetInt32(1);
                docsLength[docLengthQuery.GetString(0)] = auxLength;
                docsLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(1) - docLengthQuery.GetInt32(2) - docLengthQuery.GetInt32(3) - docLengthQuery.GetInt32(4) - docLengthQuery.GetInt32(5) - docLengthQuery.GetInt32(6) - docLengthQuery.GetInt32(7);
                contentLength[docLengthQuery.GetString(0)] = auxLength;
                contentLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(2);
                boldLength[docLengthQuery.GetString(0)] = auxLength;
                boldLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(3);
                italicLength[docLengthQuery.GetString(0)] = auxLength;
                italicLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(4);
                headerLength[docLengthQuery.GetString(0)] = auxLength;
                headerLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(5);
                titleLength[docLengthQuery.GetString(0)] = auxLength;
                titleLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(6);
                keywordsLength[docLengthQuery.GetString(0)] = auxLength;
                keywordsLengthAvg += auxLength;

                auxLength = docLengthQuery.GetInt32(7);
                descriptionLength[docLengthQuery.GetString(0)] = auxLength;
                descriptionLengthAvg += auxLength;
            }
            docLengthQuery.Close();
            docsLengthAvg /= docCol.Count();
            contentLengthAvg /= docCol.Count();
            boldLengthAvg /= docCol.Count();
            italicLengthAvg /= docCol.Count();
            headerLengthAvg /= docCol.Count();
            titleLengthAvg /= docCol.Count();
            descriptionLengthAvg /= docCol.Count();
            keywordsLengthAvg /= docCol.Count();
                ...
                if (term.Key != null)
                {
                    ...
                    if (termExist.Read())
                    {
                        ...
                        while (termsInDoc.Read())
                        {
                            ...
                            tfValue = termsInDoc.GetInt32(1) - termsInDoc.GetInt32(2) - termsInDoc.GetInt32(3) - termsInDoc.GetInt32(4) - termsInDoc.GetInt32(5) - termsInDoc.GetInt32(6) - termsInDoc.GetInt32(7);
                            wContent = ((tfValue * pesosContent) / (1 - b + b * (contentLength[docName] / contentLengthAvg)));
                            wBold = ((termsInDoc.GetInt32(2) * pesosBold) / (1 - b + b * (boldLength[docName] / boldLengthAvg)));
                            wItalic = ((termsInDoc.GetInt32(3) * pesosItalic) / (1 - b + b * (italicLength[docName] / italicLengthAvg)));
                            wHeader = ((termsInDoc.GetInt32(4) * pesosHeader) / (1 - b + b * (headerLength[docName] / headerLengthAvg)));
                            wTitle = ((termsInDoc.GetInt32(5) * pesosTitle) / (1 - b + b * (titleLength[docName] / titleLengthAvg)));
                            wKeywords = ((termsInDoc.GetInt32(6) * pesosKeywords) / (1 - b + b * (keywordsLength[docName] / keywordsLengthAvg)));
                            wDescription = ((termsInDoc.GetInt32(7) * pesosDescription) / (1 - b + b * (descriptionLength[docName] / descriptionLengthAvg)));
                            //agregamos el nominador a resultados
                            if (resultados.TryGetValue(docName, out score))
                            {
                                /*resultados[docName] = score + term.Value * (idf * (
                                    pesosContent * ((termsInDoc.GetInt32(1)) / (termsInDoc.GetInt32(1) + k * (1 - b + b * (docsLength[docName] / docsLengthAvg))))
                                    ));*/
                                resultados[docName] = score + wQuery * (idf * (
                                   (wContent + wBold + wItalic + wHeader + wTitle + wKeywords + wDescription) / (k + (wContent + wBold + wItalic + wHeader + wTitle + wKeywords + wDescription))
                                   ));
                            }
                            else
                            {
                                /* resultados[docName] = term.Value * (idf * (
                                     pesosContent * ((termsInDoc.GetInt32(1)) / (termsInDoc.GetInt32(1) + k * (1 - b + b * (docsLength[docName] / docsLengthAvg))))
                                     ));*/
                                resultados[docName] = wQuery * (idf * (
                                    (wContent + wBold + wItalic + wHeader + wTitle + wKeywords + wDescription) / (k + (wContent + wBold + wItalic + wHeader + wTitle + wKeywords + wDescription))
                                    ));
                            }
        ...
        public IRun<IListResult> RunModulo1y2(string dbConnectionString, DocumentCollection docCol, Topic topic)
        {
            ...
            {
                frecuency = 1;
                queryExpansion[regLimpiar.Replace(term.ToLower(), "")] = frecuency;
            ...
            foreach (var term in queryExpansion)
            {
                if (term.Key.Length < 128)
                {
                    ...
                    {
                        queryStemm[terminoQueryLimpio] = (1/term.Value) + frecuencyAux;
                    }
                    else
                    {
                        queryStemm[terminoQueryLimpio] =1/term.Value;
                    }
                }
            }
            int tfValue;
        ...
        public IRun<IListResult> RunModulo1y2y3(string dbConnectionString, DocumentCollection docCol, Topic topic)
        {
            ...
            int countTermsInt;
            double pesosContent = 1.0;
            double pesosBold = 60.0;
            double pesosItalic = 30.0;
            double pesosHeader = 30.0;
            double pesosTitle = 20.0;
            double pesosKeywords = 200.0;
            double pesosDescription = 10.0;
            double wContent;
            double wBold;
            double wItalic;
            double wHeader;
            ...
            if (questionEntity)
            {
                foreach (var resultado in resultados)
                {
                    ...
                    if (docEntities.Read())
                    {
                        ...
                        resultadosFinal[resultado.Key] = ((resultado.Value - resultadoMin) / (resultadoMax - resultadoMin)) * Math.Log(1.1 + numOfEntities);
                    }
                    //Si no, solo normalizamos
                    else
                    {
                        resultadosFinal[resultado.Key] = ((resultado.Value - resultadoMin) / (resultadoMax - resultadoMin)) * Math.Log(1.1);
                    }
                    docEntities.Close();
                }
            }
