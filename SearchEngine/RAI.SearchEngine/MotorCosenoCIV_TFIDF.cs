...
{
    public class MotorCosenoCIV_TFIDF : ISearchEngine
    {
        //nombre del motor
        ...
        public IRun<IListResult> RunModulo1(string dbConnectionString, DocumentCollection docCol, Topic topic)
        {
            ...
            Dictionary<string, double> raizDocumentos = new Dictionary<string, double>();
            ...
            double pesosBold = 6.0;
            double pesosItalic = 4.0;
            double pesosHeader = 4.0;
            double pesosTitle = 2.0;
            double pesosKeywords = 10.0;
            double pesosDescription = 2.0;
            ...
            foreach (var term in queryTerms)
            {
                if (term.Key != null)
                {
                    ...
                    System.Data.SqlClient.SqlDataReader termExist = termSelect.ExecuteReader();
                    if (termExist.Read())
                    {
                        ...
                        System.Data.SqlClient.SqlDataReader countTerms = nTermsInDoc.ExecuteReader();
                        countTerms.Read();
                        int countTermsInt = countTerms.GetInt32(0);
                        idf = Math.Log(docCol.Count / (countTermsInt + 1.0)); //Falta mirar como hacer un count sobre termsInDoc si se puede, y si no hacer una query Count
                        wQuery = term.Count() * idf;
                        countTerms.Close();
                        termFrecuency.Parameters["@idterm"].Value = idTerm;
                        System.Data.SqlClient.SqlDataReader termsInDoc = termFrecuency.ExecuteReader();
                        ...
                        while (termsInDoc.Read())
                        {
                            ...
                            {
                                resultados[docName] = score + ((((tfValue * pesosContent) + (termsInDoc.GetInt32(2) * pesosBold) + (termsInDoc.GetInt32(3) * pesosItalic) + (termsInDoc.GetInt32(4) * pesosHeader) + (termsInDoc.GetInt32(5) * pesosTitle) + (termsInDoc.GetInt32(6) * pesosKeywords) + (termsInDoc.GetInt32(7) * pesosDescription)) * idf) * wQuery);
                            }
                            else
                            {
                                resultados[docName] = ((((tfValue * pesosContent) + (termsInDoc.GetInt32(2) * pesosBold) + (termsInDoc.GetInt32(3) * pesosItalic) + (termsInDoc.GetInt32(4) * pesosHeader) + (termsInDoc.GetInt32(5) * pesosTitle) + (termsInDoc.GetInt32(6) * pesosKeywords) + (termsInDoc.GetInt32(7) * pesosDescription)) * idf) * wQuery);
                            }
                            //Agregamos la raiz qe wij^2 del denominador
                            if (raizDocumentos.TryGetValue(docName, out score))
                            {
                                raizDocumentos[docName] = score + Math.Pow((((tfValue * pesosContent) + (termsInDoc.GetInt32(2) * pesosBold) + (termsInDoc.GetInt32(3) * pesosItalic) + (termsInDoc.GetInt32(4) * pesosHeader) + (termsInDoc.GetInt32(5) * pesosTitle) + (termsInDoc.GetInt32(6) * pesosKeywords) + (termsInDoc.GetInt32(7) * pesosDescription)) * idf), 2);
                            }
                            else
                            {
                                raizDocumentos[docName] = Math.Pow((((tfValue * pesosContent) + (termsInDoc.GetInt32(2) * pesosBold) + (termsInDoc.GetInt32(3) * pesosItalic) + (termsInDoc.GetInt32(4) * pesosHeader) + (termsInDoc.GetInt32(5) * pesosTitle) + (termsInDoc.GetInt32(6) * pesosKeywords) + (termsInDoc.GetInt32(7) * pesosDescription)) * idf), 2);
                            }

                        }
                        raizQuery += Math.Pow(wQuery, 2);
                        termsInDoc.Close();
            ...
            conexion.Close();
            //Calculamos el valor de coseno TFIDF
            foreach (var doc in raizDocumentos)
            {
                if (resultados.TryGetValue(doc.Key, out score))
                {
                    resultados[doc.Key] = score / (Math.Sqrt(doc.Value) * Math.Sqrt(raizQuery));
            ...
            int posicion = 0;
            //Imprimimos los resultados por pantalla y falta agregar que los almacene en el formato de EIREX, que no se como va
                ...
                if (term.Key != null)
                {
                    ...
                    if (termExist.Read())
                    {
                        ...
                        while (termsInDoc.Read())
                        {
                            ...
                            if (resultados.TryGetValue(docName, out score))
                            {
                                resultados[docName] = score + term.Value * ((((tfValue * pesosContent) + (termsInDoc.GetInt32(2) * pesosBold) + (termsInDoc.GetInt32(3) * pesosItalic) + (termsInDoc.GetInt32(4) * pesosHeader) + (termsInDoc.GetInt32(5) * pesosTitle) + (termsInDoc.GetInt32(6) * pesosKeywords) + (termsInDoc.GetInt32(7) * pesosDescription)) * idf) * wQuery);
                            }
                            else
                            {
                                resultados[docName] = term.Value * ((((tfValue * pesosContent) + (termsInDoc.GetInt32(2) * pesosBold) + (termsInDoc.GetInt32(3) * pesosItalic) + (termsInDoc.GetInt32(4) * pesosHeader) + (termsInDoc.GetInt32(5) * pesosTitle) + (termsInDoc.GetInt32(6) * pesosKeywords) + (termsInDoc.GetInt32(7) * pesosDescription)) * idf) * wQuery);
                            }
                            //Agregamos la raiz qe wij^2 del denominador
                            if (raizDocumentos.TryGetValue(docName, out score))
                            {
