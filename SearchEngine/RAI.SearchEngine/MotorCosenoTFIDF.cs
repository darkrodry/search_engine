...
{
    public class MotorCosenoTFIDF : ISearchEngine
    {
        //nombre del motor
                ...
                if (term.Key != null)
                {
                    ...
                    if (termExist.Read())
                    {
                        ...
                        while (termsInDoc.Read())
                        {
                            ...
                            {
                                resultados[docName] = score + ((termsInDoc.GetInt32(1) * idf) * wQuery);
                            }
                            else
                            {
                                resultados[docName] = ((termsInDoc.GetInt32(1) * idf) * wQuery);
                            }
                            //Agregamos la raiz qe wij^2 del denominador
                            if (raizDocumentos.TryGetValue(docName, out score))
                            {
                                raizDocumentos[docName] = score + Math.Pow(termsInDoc.GetInt32(1) * idf, 2);
                            }
                            else
                            {
                                raizDocumentos[docName] = Math.Pow(termsInDoc.GetInt32(1) * idf, 2);
                            }
                ...
                if (term.Key != null)
                {
                    ...
                    if (termExist.Read())
                    {
                        ...
                        while (termsInDoc.Read())
                        {
                            ...
                            if (resultados.TryGetValue(docName, out score))
                            {
                                resultados[docName] = score + term.Value * ((termsInDoc.GetInt32(1) * idf) * wQuery);
                            }
                            else
                            {
                                resultados[docName] = term.Value * ((termsInDoc.GetInt32(1) * idf) * wQuery);
                            }
                            //Agregamos la raiz qe wij^2 del denominador
                            if (raizDocumentos.TryGetValue(docName, out score))
                            {
